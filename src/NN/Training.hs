...
-- For BCE+sigmoid: dL/dz = pred - expect
cost' :: Double -> Double -> Double
...
trainOnAll epsilon network inputs = 
    foldl' (\net input -> train epsilon 1 net input) network inputs
...
trainComprehensive epsilon maxIterations network inputs = 
    foldl' (\net _ -> trainOnAll epsilon net inputs) network [1..maxIterations `div` length inputs]
